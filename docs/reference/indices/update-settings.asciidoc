...
====== Note:
When the update request body has a field named 'settings', all the fields are
parsed with no special handling for the 'settings' field.
In the below example, the settings properties parsed are : field1, field2 and
settings.field3 ( this throws during validation )
