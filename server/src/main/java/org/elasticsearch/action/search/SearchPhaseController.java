...
public final class SearchPhaseController {
    ...
    public InternalSearchResponse merge(boolean ignoreFrom, ReducedQueryPhase reducedQueryPhase,
        ...
        SearchHits hits = reducedQueryPhase.getHits(ignoreFrom, fetchResults, resultsLookup);
    ...
    ReducedQueryPhase reducedScrollQueryPhase(Collection<? extends SearchPhaseResult> queryResults) {
        ...
        return topDocsStats.reducedQueryPhase(queryResults, Collections.emptyList(), topDocs,
    ...
    public static final class ReducedQueryPhase {
        ...
        private SearchHits getHits(boolean ignoreFrom,
            ...
            int from = ignoreFrom ? 0 : this.from;
            int numSearchHits = (int) Math.min(fetchHits - from, size);
            ...
            if (fetchResults.isEmpty() == false) {
                for (int i = 0; i < numSearchHits; i++) {
                    ...
                    if (sortedTopDocs.isSortedByField) {
                        ...
                        searchHit.sortValues(fieldDoc.fields, sortValueFormats);
                        if (sortScoreIndex != -1) {
            ...
            return new SearchHits(hits.toArray(new SearchHit[0]), totalHits,
                maxScore, sortedTopDocs.sortFields, sortedTopDocs.collapseField, sortedTopDocs.collapseValues);
        }
    }
    ...
    static final class TopDocsStats {
         ...
         * @param isScrollRequest
         * @param aggReduceContextBuilder
         * @param performFinalReduce
        ...
        ReducedQueryPhase reducedQueryPhase(Collection<? extends SearchPhaseResult> queryResults,
                                            ...
                                            int numReducePhases, boolean isScrollRequest,
                                            InternalAggregation.ReduceContextBuilder aggReduceContextBuilder,
                                            boolean performFinalReduce) {
            assert numReducePhases >= 0 : "num reduce phases must be >= 0 but was: " + numReducePhases;
            numReducePhases++; // increment for this phase
            if (queryResults.isEmpty()) { // early terminate we have nothing to reduce
                final TotalHits totalHits = getTotalHits();
            ...
            final TotalHits totalHits = getTotalHits();
            return new ReducedQueryPhase(totalHits, fetchHits, getMaxScore(),
                timedOut, terminatedEarly, reducedSuggest, aggregations, profileBuilder, sortedTopDocs,
